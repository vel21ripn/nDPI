	...
	(automa == ndpi_str->host_automa.ac_automa ||
	 automa == ndpi_str->content_automa.ac_automa)) {
	  ...
	  if(protocol_id == ac_pattern.rep.number) {
		  ...
		  NDPI_LOG_ERR(ndpi_str, "[NDPI] Missmatch '%s' proto %s origin %s\n",
				  value, tproto,
				  ndpi_get_proto_by_id(ndpi_str,ac_pattern.rep.number));
	  }
  }
...
int ndpi_init_protocol_match(struct ndpi_detection_module_struct *ndpi_str, ndpi_protocol_match *match)
{
...
struct ndpi_detection_module_struct *ndpi_init_detection_module(ndpi_init_prefs prefs)
{
    ...
    ndpi_str->content_automa.ac_automa = ac_automata_init();
...
#else
    ndpi_str->custom_categories.hostnames.ac_automa = ac_automata_init();
    ndpi_str->custom_categories.hostnames_shadow.ac_automa = ac_automata_init();
#endif
...
int ndpi_match_string(void *_automa, char *string_to_match) {
  ...
  rc = ac_automata_search(automa, &ac_match, &ac_input_text, ac_match_handler,&match);
                                ...
                                NDPI_SELECTION_BITMASK_PROTOCOL_SIZE *ndpi_selection_packet)
{
    ...
    proto_index = ndpi_str->proto_defaults[flow->guessed_protocol_id].protoIdx;
    proto_id = ndpi_str->proto_defaults[flow->guessed_protocol_id].protoId;
...
typedef void (*ndpi_callback_protocol) (struct ndpi_detection_module_struct *, struct ndpi_flow_struct *flow);

void check_ndpi_udp_flow_func(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow,
                              NDPI_SELECTION_BITMASK_PROTOCOL_SIZE *ndpi_selection_packet)
{
    ndpi_callback_protocol func = NULL;
    u_int32_t a;
                                            ...
                                            struct ndpi_id_struct *src, struct ndpi_id_struct *dst)
{
  ...
  flow->packet.tick_timestamp = get_timestamp(current_tick_l,ndpi_str->ticks_per_second);
	 ...
	 */
#ifdef SKIP_INVOKING_THE_DISSECTORS
	if(flow->guessed_host_protocol_id != NDPI_PROTOCOL_UNKNOWN) {
	  /*
