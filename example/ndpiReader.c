  ...
  *proto = iph->protocol;
  ...
  if(ret == NULL) {
	...
	return(NULL);
      ...
      newflow->s_port = s_port; newflow->d_port = d_port;
	...
	memset(newflow->ndpi_flow, 0, size_flow_struct);
      if(version == 4) {
		...
		return(NULL);
      	      ...
      	      if((newflow->src_id = ndpi_id6_alloc(&iph6->ip6_src.u6_addr.u6_addr32[0])) == NULL) {
		printf("[NDPI] %s(3): not enough memory\n", __FUNCTION__);
		return(NULL);
	      } 

	      if((newflow->dst_id = ndpi_id6_alloc(&iph6->ip6_dst.u6_addr.u6_addr32[0])) == NULL) {
		printf("[NDPI] %s(4): not enough memory\n", __FUNCTION__);
		return(NULL);
	      }
      }
	...
	if(version == 4) {
		...
		flow->s_port == s_port && flow->d_port == d_port)
	      ...
	      *src = &(flow->dst_id)->ndpi_id, *dst = &(flow->src_id)->ndpi_id, *src_to_dst_direction = 1;
	} else {
